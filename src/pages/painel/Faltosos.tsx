...
const Faltosos: React.FC = () => {
  ...
  const canAccess =
    ["master", "gestor", "admin", "coordenador", "recepcao", "profissional"].includes(role) || isGlobalAdmin;
  ...
  const fetchData = useCallback(async () => {
    setLoading(true);
    try {
      const { data, error } = await supabase.from("paciente_profissional_status").select(`
          paciente_id,
          profissional_id,
          total_faltas,
          status_falta,
  ...
  if (!canAccess) {
    return (
      <div className="p-10 text-center text-muted-foreground">Você não tem permissão para acessar esta página.</div>
    );
  }
  ...
  return (
    <div className="space-y-5 p-5">
      <PageHeader
        title="Pacientes Faltosos / Bloqueados"
        subtitle="Controle de faltas injustificadas por vínculo Profissional + Paciente"
      />
      ...
      <Card>
        <CardContent className="p-4 grid grid-cols-1 md:grid-cols-6 gap-3 items-end">
          ...
          <div className="flex items-center justify-between gap-2 h-9 border rounded-md px-2">
            <Label htmlFor="excecao" className="text-[10px] leading-tight">
              Exceções (TFD/OJ)
            </Label>
                ...
                {filtradas.map((r) => {
                  ...
                  return (
                    <TableRow key={r.id}>
                        ...
                        <div className="flex flex-col">
                          <span className="flex items-center gap-2">
                            ...
                            {excecao && (
                              <Badge
                                variant="outline"
                                className="border-blue-300 text-blue-700 bg-blue-50 py-0 text-[10px]"
                              >
                      ...
                      <TableCell className="text-center font-bold">{r.total_faltas}</TableCell>
