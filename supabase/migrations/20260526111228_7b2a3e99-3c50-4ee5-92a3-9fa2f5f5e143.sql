-- 1. Remover gatilho redundante que causava inconsistência
DROP TRIGGER IF EXISTS tr_refresh_pps ON public.agendamentos;

-- 2. Atualizar refresh_paciente_profissional_status para ser completo
...
BEGIN
    -- Carregar limites da configuração
    ...
    -- Inserir dados consolidados de ambas as fontes
    ...
    FROM (
        ...
        -- Faltas da Gestão de Tratamento (Sessões)
        SELECT 
    ...
    -- Adicionar entradas REGULAR para pacientes isentos (TFD/OJ)
    INSERT INTO public.paciente_profissional_status (
        paciente_id,
        profissional_id,
        total_faltas,
    ...
    SELECT DISTINCT
        p.id as paciente_id,
        curr.profissional_id,
        0,
        'REGULAR',
        NULL::DATE
    FROM public.pacientes p
    CROSS JOIN (
        SELECT DISTINCT profissional_id FROM public.agendamentos
        UNION
        SELECT DISTINCT professional_id FROM public.treatment_sessions
    ) curr
...
-- 3. Corrigir atualizar_status_falta (versão com profissional)
...
BEGIN
  ...
  LOOP
    ...
    INSERT INTO public.paciente_profissional_status (
      ...
      p_paciente_id, curr_prof.prof_id, v_total, v_novo_status, v_ultima_falta, now()
...
-- 4. Rodar o refresh inicial para corrigir dados legados
