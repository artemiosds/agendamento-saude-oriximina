...
CREATE OR REPLACE FUNCTION public.save_document_template(
  ...
  p_tipo_modelo text DEFAULT 'UNIDADE',
...
BEGIN
  ...
  ELSIF v_is_global_owner THEN
    v_target_unidade := COALESCE(v_target_unidade, '');
  ELSE
  ...
  IF p_template_id IS NOT NULL THEN
    ...
    IF NOT v_is_global_owner THEN
      ...
      IF v_is_manager THEN
        IF COALESCE(v_existing.unidade_id, '') <> v_staff.unidade_id THEN
    ...
    UPDATE public.document_templates
       SET nome = trim(p_nome),
           ...
           tipo_modelo = CASE WHEN v_is_global_owner THEN v_tipo_modelo ELSE v_existing.tipo_modelo END,
           unidade_id = CASE WHEN v_is_global_owner THEN v_target_unidade ELSE v_existing.unidade_id END,
           blocos_clinicos = COALESCE(p_blocos_clinicos, '[]'::jsonb),
           versoes = COALESCE(p_versoes, '[]'::jsonb),
           updated_at = now()
     WHERE id = p_template_id
