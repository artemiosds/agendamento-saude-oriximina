...
AS $function$
 ...
 BEGIN
   ...
   WITH base AS (
     ...
     WHERE
       (p_professional_id IS NULL OR p_professional_id = '' OR c.professional_id = p_professional_id)
       AND (p_unit_id IS NULL OR p_unit_id = '' OR c.unit_id = p_unit_id)
       AND (
       ...
       AND (
         ...
         unaccent(f.nome) ILIKE '%' || unaccent(p_search) || '%' OR
         unaccent(c.status) ILIKE '%' || unaccent(p_search) || '%' OR
         -- Common status label translations
         (unaccent(p_search) ILIKE '%andamento%' AND c.status = 'em_andamento') OR
         (unaccent(p_search) ILIKE '%concluido%' AND c.status = 'concluido') OR
         (unaccent(p_search) ILIKE '%fila%' AND c.status = 'em_fila') OR
         (unaccent(p_search) ILIKE '%vaga%' AND c.status = 'aguardando_vaga') OR
         (unaccent(p_search) ILIKE '%alta%' AND c.status = 'finalizado_alta') OR
         (unaccent(p_search) ILIKE '%suspenso%' AND c.status = 'suspenso')
       )
   ),
   counted AS (
